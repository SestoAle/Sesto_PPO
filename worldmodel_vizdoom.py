...
from architectures.vizdoom_arch import *
...
class WorlModelCanvas(QObject, scene.SceneCanvas):
    ...
    def trajectories_to_pos_buffer(self, trajectories, rnd=None):
        ...
        for traj in list(trajectories.values())[:]:
            ...
            for state in traj:
                ...
                position[0] = (((position[0] + 1) / 2) * 1600)
                position[1] = (((position[1] + 1) / 2) * 1150)
                position[2] = (((position[2] + 1) / 2) * 100)
    ...
    def move_agent_at_traj_index(self, index):
        ...
        smoothed_traj[:, 0] = (((smoothed_traj[:, 0] + 1) / 2) * 1600)
        smoothed_traj[:, 1] = (((smoothed_traj[:, 1] + 1) / 2) * 1150)
        smoothed_traj[:, 2] = (((smoothed_traj[:, 2] + 1) / 2) * 100)
    ...
    def move_agent(self):
        ...
        to_move[0] = (((to_move[0] + 1) / 2) * 1600)
        to_move[1] = (((to_move[1] + 1) / 2) * 1150)
        to_move[2] = (((to_move[2] + 1) / 2) * 100)
        ...
        if motivation is not None:
            ...
            for keys, traj in zip(list(trajectories.keys())[:], list(trajectories.values())[:]):
                ...
                for i, point in enumerate(traj):
                    de_point = np.zeros(3)
                    de_point[0] = (((np.asarray(point[0]) + 1) / 2) * 1600)
                    de_point[1] = (((np.asarray(point[1]) + 1) / 2) * 1150)
                    de_point[2] = (((np.asarray(point[2]) + 1) / 2) * 100)
            ...
            for key, traj, idx_traj in zip(episodes_to_observe, traj_to_observe, range(len(traj_to_observe))):
                ...
                for state, action in zip(traj, actions[key]):
                    ...
                    de_point = np.zeros(3)
                    de_point[0] = ((np.asarray(state['global_in'][0]) + 1) / 2) * 1600
                    de_point[1] = ((np.asarray(state['global_in'][1]) + 1) / 2) * 1150
                    de_point[2] = ((np.asarray(state['global_in'][2]) + 1) / 2) * 100
    ...
    def print_3d_traj(self, traj, index=None, with_denorm=True, max=None, min=None, tmp_line=False, im_rews=None,
        ...
        if with_denorm:
            ep_trajectory[:, 0] = ((np.asarray(ep_trajectory[:, 0]) + 1) / 2) * 1600
            ep_trajectory[:, 1] = ((np.asarray(ep_trajectory[:, 1]) + 1) / 2) * 1150
            ep_trajectory[:, 2] = ((np.asarray(ep_trajectory[:, 2]) + 1) / 2) * 100
