...
def network_spec(states):
    ...
    if input_length > 71:
        ...
        if with_circular:
            ...
            global_grid = embedding(global_grid, indices=7, size=32, name='global_embs')
            ...
            rays = embedding(rays, indices=6, size=32, name='rays_embs')
        ...
        # Jump
        agent, goal, grid, rays, inventory = tf.split(global_state, [3, 5, 49, 12, 2], axis=1)
...
def network_spec_rnd(states):
    ...
    # agent, goal, rays, obs = tf.split(global_state, [4, 3, 12, 21], axis=1)
    # Jump
    agent, goal, grid, rays, inventory = tf.split(global_state, [2, 6, 49, 12, 2], axis=1)
...
def network_spec_irl(states, states_n, act, with_action, actions_size):
    ...
    # Jump
    agent, goal, grid, rays, inventory = tf.split(global_state, [2, 6, 49, 12, 2], axis=1)
    agent_n, goal_n, grid_n, rays_n, inventory_n = tf.split(global_state_n, [2, 6, 49, 12, 2], axis=1)
