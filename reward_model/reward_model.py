        ...
        with tf.compat.v1.variable_scope(self.name) as vs:
            with tf.compat.v1.variable_scope('irl'):
                ...
                if self.gradient_penalty_weight > 0.0:
                    self.expert_states = tf.compat.v1.placeholder(tf.float32, [None, 73], name='exp_state')
                    self.expert_acts = tf.compat.v1.placeholder(tf.int32, [None, 1], name='expert_acts')
                    self.expert_states_n = tf.compat.v1.placeholder(tf.float32, [None, 73], name='exp_state_n')
