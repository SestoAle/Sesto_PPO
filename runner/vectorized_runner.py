...
class Runner:
    ...
    def clear_vec_buffer(self):
        self.vec_buffer_per_agent = []
        for a in range(self.env.num_agents):
            self.vec_buffer_per_agent.append(
                dict(state=[], state_n=[], action=[], reward=[], logprob=[], done=[])
            )

    def add_to_vec_buffer(self, states, states_n, actions, rewards, logprobs, dones):
        for a in range(self.env.num_agents):
            self.vec_buffer_per_agent[a]["state"].append(states[a])
            self.vec_buffer_per_agent[a]["state_n"].append(states_n[a])
            self.vec_buffer_per_agent[a]["action"].append(actions[a])
            self.vec_buffer_per_agent[a]["reward"].append(rewards[a])
            self.vec_buffer_per_agent[a]["logprob"].append(logprobs[a])
            self.vec_buffer_per_agent[a]["done"].append(dones[a])
    ...
    def run(self):
        ...
        # Utils for vectorized env
        self.clear_vec_buffer()
        ...
        while self.ep <= self.total_episode:
            ...
            while True:
                ...
                if self.motivation is not None:
                    ...
                    for s_n in state_n:
                        self.motivation.add_to_buffer(s_n)
                ...
                # Update PPO memory
                if not self.recurrent:
                    self.add_to_vec_buffer(state, state_n, action, reward, logprob, done)
                else:
                    try:
                ...
                if all(done):
                    ...
                    # At the end of the episode, save the vec_buffer into the real buffer
                    if self.frequency_mode == 'episodes':
                        for a in range(self.env.num_agents):
                            for state, state_n, action, reward, logprob, done in zip(
                                    self.vec_buffer_per_agent[a]['state'],
                                    self.vec_buffer_per_agent[a]['state_n'],
                                    self.vec_buffer_per_agent[a]['action'],
                                    self.vec_buffer_per_agent[a]['reward'],
                                    self.vec_buffer_per_agent[a]['logprob'],
                                    self.vec_buffer_per_agent[a]['done'],
                            ):
                                self.agent.add_to_buffer(state, state_n, action, reward, logprob, done)
                        self.clear_vec_buffer()

                    self.history['episode_rewards'].extend(episode_reward)
                    self.history['episode_timesteps'].append(step*self.env.num_agents)
